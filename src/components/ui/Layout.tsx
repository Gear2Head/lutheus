...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  // Get brand gradient class based on selected theme
  ...
  return (
    <div className="flex h-screen w-full overflow-hidden select-none bg-[#050506]">
      ...
      {/* Sleek Glassmorphic sidebar with exact dimensions */}
      <aside className={getSidebarClass()}>
        ...
        <nav className={`flex-1 overflow-y-auto hide-scrollbar py-6 ${isCollapsed ? 'px-2 space-y-3' : 'px-3 space-y-1.5'} select-none`}>
          <NavItem href="/" icon={LayoutGrid} label="Ana Sayfa" isCollapsed={isCollapsed} isActive={pathname === '/'} />
          <NavItem href="/penalties" icon={Shield} label="Cezalar" isCollapsed={isCollapsed} isActive={pathname === '/penalties'} />
          <NavItem href="/staff" icon={Users} label="Yetkililer" isCollapsed={isCollapsed} isActive={pathname === '/staff'} />
          <NavItem href="/pointtrain" icon={Zap} label="Pointtrain" isCollapsed={isCollapsed} isActive={pathname === '/pointtrain'} />
          <NavItem href="/editor" icon={BookOpen} label="CUK Editörü" isCollapsed={isCollapsed} isActive={pathname === '/editor'} />
          <NavItem href="/agent" icon={Activity} label="AI Agent" isCollapsed={isCollapsed} isActive={pathname === '/agent'} />
          <NavItem href="/access" icon={Layers} label="Erişim" isCollapsed={isCollapsed} isActive={pathname === '/access'} />
          <NavItem href="/announcements" icon={MessageSquare} label="Duyurular" isCollapsed={isCollapsed} isActive={pathname === '/announcements'} />
          {isCollapsed && (
            <button 
              onClick={toggleSidebar}
        ...
        {/* User Card with Interactive Bottom-Left Dropdown Menu */}
            ...
            {profileMenuOpen && (
              ...
              >
                ...
                <button 
                  onClick={() => {
                    setProfileMenuOpen(false);
                    alert('Profil Detayları modülü tetiklendi.');
                  }}
                ...
                <button 
                  onClick={() => {
                    setProfileMenuOpen(false);
                    alert('Bildirimler merkezine ulaşıldı.');
                  }}
                  className="w-full flex items-center gap-2.5 px-3 py-2 rounded-lg hover:bg-white/5 text-white/70 hover:text-white transition-colors text-left text-[12px] font-semibold cursor-pointer"
                >
      ...
      {/* Main Content Pane with high fidelity Apple custom transition animations */}
      ...
      {/* Global Notification Center Slider */}
