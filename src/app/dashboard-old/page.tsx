...
export default function Dashboard() {
  ...
  return (
    <motion.div 
      ...
      className="p-6 md:p-8 w-full space-y-6 md:space-y-8 select-none"
    >
      ...
      <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 border-b border-white/[0.04] pb-6">
        <div>
          ...
          <p className="text-[13px] text-[#8E8E93] mt-1 font-medium">
            Gerçek zamanlı veri akışı • <span className="text-white/60 font-semibold">{totalPenalties} kayıt analiz edildi</span>
          </p>
        </div>
        ...
        <div className="flex items-center gap-2 self-start sm:self-auto">
          ...
          <motion.button 
            ...
            className="px-4 py-2 rounded-lg bg-white/[0.04] border border-white/[0.08] hover:bg-white/[0.08] text-[12.5px] font-semibold text-white/95 transition-colors cursor-pointer"
          >
            Özet Rapor Kopyala
          </motion.button>
          ...
          <motion.button 
            ...
            disabled={isLoading}
            className="px-4 py-2 rounded-lg bg-white/[0.04] border border-white/[0.08] text-white hover:bg-white/[0.08] text-[12.5px] font-bold transition-all flex items-center gap-1.5 cursor-pointer disabled:opacity-55 animate-none"
          >
            <RotateCw size={12} className={isLoading ? 'animate-spin text-[#5E5CE6]' : ''} /> Yenile
          </motion.button>
        </div>
      </div>
          ...
          >
            <div className="flex items-center justify-between mb-4 pb-3 border-b border-white/[0.03]">
              <div className="flex items-center gap-2.5">
                ...
                <div>
                  <h3 className="text-[14px] font-black text-white tracking-tight">Son Moderasyon Kararları</h3>
                  <p className="text-[11px] text-[#8E8E93] mt-0.5">Gerçek zamanlı doğrulama ve denetleme akışı</p>
                </div>
              </div>
              
              <button 
                onClick={() => router.push('/penalties')}
                className="text-[11px] font-bold text-[#5E5CE6] hover:text-[#A259FE] hover:underline flex items-center gap-1 cursor-pointer transition-colors"
              >
            ...
            <div className="space-y-2.5">
              {recentModerations.map((log, index) => {
                ...
                return (
                  <div 
                    key={log.id + index}
                    className="flex flex-col sm:flex-row sm:items-center justify-between p-3.5 rounded-xl bg-[#111112]/30 border border-white/[0.03] hover:bg-white/[0.02] transition-colors gap-4"
                  >
                    {/* User and Staff Details */}
                    <div className="flex items-center gap-3 min-w-0">
                      <div className="relative shrink-0">
                        <img src={log.avatar} alt="avatar" className="w-8 h-8 rounded-lg border border-white/10 object-cover" />
                        <span className={`absolute -bottom-0.5 -right-0.5 w-2 h-2 rounded-full border border-black ${
                          log.status === 'DOĞRU' ? 'bg-[#32D74B]' : 'bg-[#FF453A]'
                        }`} />
                      </div>
                      
                      <div className="truncate space-y-0.5">
                        <div className="flex items-center gap-2 flex-wrap">
                          <span className="text-[12.5px] font-bold text-white tracking-tight">{log.staff}</span>
                          <span className="text-[10px] font-mono font-semibold text-[#5E5CE6]/90 px-1 py-0.2 px-1.5 rounded bg-[#5E5CE6]/5">
                            {log.id}
                          </span>
                        </div>
                        <p className="text-[12px] text-white/60 truncate font-medium max-w-[280px]">
                          {log.reason}
                        </p>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center gap-4 flex-wrap sm:flex-nowrap justify-between sm:justify-end">
                      <div className="text-left sm:text-right">
                        <span className="text-[11.5px] font-mono font-bold text-white/50 block">
                          {log.duration || 'Belirsiz'}
                        </span>
                        <span className="text-[9.5px] text-[#8E8E93] block mt-0.5 font-medium">
                          {log.date}
                        </span>
                      </div>
